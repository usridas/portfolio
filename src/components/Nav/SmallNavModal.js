    ...
    <navigation className='smallNavModalContainer'>
      <div className="smallNavModal">
        <div className='smallNavList'>
          ...
          <button tabIndex={0} className='link' style={{textDecorationLine: location.pathname==='/Work' ? 'none' : 'underline'}} onClick={()=>{navigate('/Work'); setIsModalOpen(false);}} onKeyDown={(event)=>{if (event.key === 'Enter') {navigate('/Work'); setIsModalOpen(false);}}}>Work</button>
