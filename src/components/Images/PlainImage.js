...
export const PlainImage = ({plainImageProps}) => {
  ...
  return (
    <div className="imageSlideContainer">
        ...
        <img onLoad={()=>setIsLoading(false)} alt={toSentenceCase(plainImageProps.title)} aria-label={toSentenceCase(plainImageProps.title)} className='plainImage' src={plainImageProps.imagePath} style={isLoading ? { display: 'none' } : {}}/>
